...
namespace FoodMart_Pro.Admin
{
    public partial class ManageProduct : System.Web.UI.Page
    {
        ...
        void select()
        {
            ...
            ddlCategory.SelectedValue = ds.Tables[0].Rows[0][4].ToString();
        }
        ...
        protected void btnAddProduct_Click(object sender, EventArgs e)
        {
            ...
            if (btnAddProduct.Text == "Add Product")
            {
                ...
                "INSERT INTO Products_tbl(ProductName, Weight, Price, CategoryID, CategoryName, ProductImage) " +
                "VALUES('" + txtProductName.Text + "', '" + txtWeight.Text + "', '" + txtPrice.Text + "', '" + ddlCategory.SelectedValue + "', '" + ddlCategory.SelectedItem.Text + "', '" + fnm + "')",
                conn);
            ...
            else
            {
                getcon();
                conn.Open();
                cmd = new SqlCommand("UPDATE Products_tbl SET ProductName='" + txtProductName.Text +
                                     "', Weight='" + txtWeight.Text +
                                     "', Price='" + txtPrice.Text +
                                     "' WHERE ProductID='" + ViewState["id"] + "'", conn);
                cmd.ExecuteNonQuery();
                conn.Close();
                fillgrid();
                clear();
